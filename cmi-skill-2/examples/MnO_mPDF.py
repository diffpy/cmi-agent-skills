...
svec = np.array(
    [1.0, -1.0, 0.0]
)  # spins point parallel or anti-parallel to this direction
...
uc = struc[:4].xyz
visatoms = uc.copy()
visatoms = np.concatenate(
    (
        visatoms,
        visatoms + np.array([0, 0, 1]),
        visatoms + np.array([0, 1, 0]),
        visatoms + np.array([0, 0, 1]),
    )
)
...
mc = MPDFcalculator(
    mstr
)  # load the magnetic structure into the MPDFcalculator
...
ax.plot(
    r, d, "bo", label="mPDF data", markerfacecolor="none", markeredgecolor="b"
)
...
ax.plot(
    r, gobs, marker="o", mfc="none", mec="b", linestyle="none"
)  ### total observed PDF pattern
ax.plot(r, gcalc, "r-", lw=2)  ### calculated atomic PDF
ax.plot(
    r, d + offset1, marker="o", mfc="none", mec="b", linestyle="none"
)  ### residual from atomic PDF fit
ax.plot(r, fit + offset1, "r-", lw=2)  ### calculated mPDF
ax.plot(r, np.zeros_like(r) + offset2, "k:")
ax.plot(
    r, mdiff + offset2, "g-"
)  ### overall fit residual after including atomic and magnetic PDFs
